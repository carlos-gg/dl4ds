...
class Involution(tf.keras.layers.Layer):
    ...
    References
    ----------
    [1] Involution: Inverting the Inherence of Convolution for Visual 
    Recognition (Li et al. 2021): https://arxiv.org/abs/2103.06255

    Notes
    -----
    Initial version borrowed from: https://keras.io/examples/vision/involution/
    """
    def __init__(self, channel, group_number, kernel_size, stride, 
                 reduction_ratio, name, full_output=False):
        super().__init__(name=name)
    ...
    def build(self, input_shape):
        ...
        self.stride_layer = (
            tf.keras.layers.AveragePooling2D(pool_size=self.stride, strides=self.stride, padding="same")
            if self.stride > 1
            else tf.identity)
        # Define the kernel generation layer.
        self.kernel_gen = tf.keras.Sequential([
            tf.keras.layers.Conv2D(filters=self.channel // self.reduction_ratio, kernel_size=1),
        ...
        self.kernel_reshape = tf.keras.layers.Reshape(
            target_shape=(height, width, self.kernel_size * self.kernel_size, 1, self.group_number)
        )
        self.input_patches_reshape = tf.keras.layers.Reshape(
            target_shape=(height, width, self.kernel_size * self.kernel_size,
                num_channels // self.group_number, self.group_number))
        self.output_reshape = tf.keras.layers.Reshape(target_shape=(height, width, num_channels))
